...
public:
    ...
    uint32_t bit_num() const {
        return _data_len * sizeof(uint64_t) * 8;
    }
...
public:
    ...
    void add_bytes(const char* buf, uint32_t len) {
        uint64_t hash = buf == nullptr ?
                BLOOM_FILTER_NULL_HASHCODE : HashUtil::hash64(buf, len, DEFAULT_SEED);
        add_hash(hash);
    }
    ...
    bool merge(const BloomFilter& that) {
        if (_bit_num == that.bit_num()
                && _hash_function_num == that.hash_function_num()) {
    ...
    uint32_t hash_function_num() const {
        return _hash_function_num;
    }

    const BitSet& bit_set() const {
        return _bit_set;
    }

    uint64_t* bit_set_data() const {
        return _bit_set.data();
    }

    uint32_t bit_set_data_len() const {
        return _bit_set.data_len();
    }

    // Convert BloomFilter to string to convenient debug and test
    std::string to_string() const {
        std::stringstream bf_stream;
        bf_stream << "bit_num:" << _bit_num
                  << " hash_function_num:" << _hash_function_num

...
public:
    BloomFilterPageBuilder(const TypeInfo* type_info, size_t bloom_filter_block_size, double fp_rate)
            : _type_info(type_info), _block_size(bloom_filter_block_size) {
        PageBuilderOptions opts;
        // do not limit the page size
        opts.data_page_size = 0;
        _page_builder.reset(new BinaryPlainPageBuilder(opts));

        _bf_builder.reset(new BloomFilterBuilder(_block_size, fp_rate));
    }
...
private:
    ...
    std::unique_ptr<BloomFilterBuilder> _bf_builder;
};
...
public:
    ...
    const BloomFilter& get_bloom_filter(int index) {

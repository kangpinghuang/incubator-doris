...
Status SegmentIterator::_get_row_ranges_from_zone_map(RowRanges* zone_map_row_ranges) {
    ...
    for (auto& column_condition : _opts.conditions->columns()) {
        ...
        LOG(INFO) << "get row ranges by zone map. row ranges:" << column_zone_map_row_ranges.to_string();
...
Status SegmentIterator::_get_row_ranges_from_bloom_filter(RowRanges* bf_row_ranges) {
    ...
    for (auto& column_condition : _opts.conditions->columns()) {
        ...
        if (!_segment->_column_readers[column_id]->has_bloom_filter()) {
            ...
            LOG(INFO) << "has no bf for column:" << column_id;
            continue;
        }   
        ...
        LOG(INFO) << "get row ranges by bf. row ranges:" << column_bf_row_ranges.to_string() << ", column_id:" << column_id;
